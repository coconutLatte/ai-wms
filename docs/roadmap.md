...
| P3-34 | P2 | PDA: Order lookup / detail page | pending | — | New PDA page: scan/type order number → fetch order with lines → display status badge, type, priority, line item table (SKU, qty, fulfilled qty). APIs exist from P1-14. i18n zh-CN+en |
| P3-35 | P2 | Admin: Audit log viewer page | pending | — | Table with filters (action, user, resource, date range). Detail modal showing old/new values JSON. Audit log API exists from P2-15 (GET /api/v1/audit-logs). i18n zh-CN+en |
...
| P3-38 | P1 | PDA: Putaway workflow page | pending | — | New PDA page: scan location barcode → find pending putaway tasks → confirm putaway with qty validation → complete task with inventory effects. Complements Receiving (P3-29) completing the inbound flow. Builds on task API (P1-15), task completion (P3-21), and barcode scanning (P3-26). i18n zh-CN+en. |
| P3-39 | P1 | PDA: Picking workflow page | pending | — | New PDA page: view pick tasks → scan location barcode → scan SKU barcode → enter picked qty → confirm → complete. Builds on task API (P1-15), task completion (P3-21), and barcode scanning (P3-26). i18n zh-CN+en. |
| P3-40 | P1 | Order cancellation: release reservations + cancel tasks | pending | — | New CancelOrder service method: validate status transition to cancelled → release all inventory reservations atomically → cancel all pending tasks for the order → update order status. All within TxManager. New API endpoint PUT /orders/{id}/cancel. Unit tests. |
...
| P3-43 | P2 | API handler unit test expansion | pending | — | Add dedicated unit tests for api/helpers.go and api/response.go. Current api package coverage: 9.6% (integration tests use build tags). Target: >50% on non-integration unit tests. |
| P3-44 | P2 | Docker Compose: PostgreSQL integration test setup | pending | — | Create docker-compose.test.yml with PostgreSQL container. Add `make test-integration` target that starts Postgres, runs go test with integration build tag against real DB. Verify postgres repo tests actually hit the database. |
| P3-45 | P2 | PDA: Network error handling & retry UX | pending | — | Add React error boundaries to all PDA pages. Show offline indicator when API calls fail. Add retry buttons on network errors. Auto-retry on token refresh. Improve loading/empty/error state consistency across all PDA pages. |

<!-- DISCOVER round 71 on 2026-07-23: refilled from 2→10 pending. Added 8 tasks: PDA putaway/picking workflows (P1, complete inbound+outbound flows), order cancellation (P1, release reservations+atomically cancel tasks), PDA stock inquiry/cycle count pages (P2), API handler unit test expansion (P2), Docker Compose test setup (P2), PDA error handling (P2). -->
