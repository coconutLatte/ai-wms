...
| P3-29 | P1 | PDA: Receiving workflow page | pending | — | New PDA page for ASN receiving: scan ASN barcode → fetch ASN → view status and line items → receive each line with qty validation → confirm receipt (ASN status to receiving/received). Auto-generates putaway tasks. ASN endpoints exist from P3-16; task generation from P3-20. i18n zh-CN+en |
| P3-30 | P2 | Admin: Wave management pages | pending | — | Wave list table with type/status filters; create wave modal (name, type, warehouse); wave detail with order list sidebar; add/remove orders (only in created status); release wave button; status transitions. Wave API already exists from P3-14 |
...
| P3-33 | P2 | Inventory transaction history API + Admin page | pending | — | Add GET /api/v1/inventory-transactions with global filters (sku_id, warehouse_id, type, date_from, date_to). Build Admin page with type badges, delta_qty, date; type/date filters; detail modal. i18n zh-CN+en |
...
<!-- GROOMED round 70 on 2026-07-23: pruned 10→8 pending. Cancelled P3-33 (Redis — premature opt) + P3-35 (gRPC — speculative). Re-sorted by impact: P1 UI pages → P1 PDA workflow → P2 UI pages → P2 quality. -->
