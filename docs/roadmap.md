...
| P3-30 | P1 | Admin: Location & Zone management pages | pending | — | Zone list with warehouse filter, create/edit zone modal (zone type selector). Location list with zone filter, create/edit location modal (type, capacity). Status transitions for locations. APIs already exist via WarehouseHandler GET/POST zones, GET/POST locations, PATCH location status. i18n zh-CN+en |
| P3-31 | P1 | PDA: Receiving workflow page | pending | — | New PDA page for ASN receiving: scan ASN barcode → fetch ASN → view status and line items → receive each line with qty validation → confirm receipt (ASN status to receiving/received). Auto-generates putaway tasks. ASN endpoints exist from P3-16; task generation from P3-20. i18n zh-CN+en |
| P3-32 | P1 | Repository integration tests | pending | — | Integration tests for postgres repos using real test DB. Cover warehouse/zone/location CRUD, inventory create/query/update/status, order lifecycle (create→confirm→fulfill), task lifecycle (create→assign→complete), ASN lifecycle, wave operations. Target 30%+ repo coverage (from 1.6%). Use t.Cleanup for teardown |
| P3-33 | P2 | Redis caching for hot inventory queries | pending | — | Integrate Redis into InventoryService: cache-aside for GetInventory, QueryInventory. TTL 60s. Invalidate on inventory changes (create, adjust, status). Add cache hit/miss logging. Redis client bootstrapped from P3-11 |
| P3-34 | P2 | Inventory transaction history API + Admin page | pending | — | Add GET /api/v1/inventory-transactions with global filters (sku_id, warehouse_id, type, date_from, date_to). Build Admin page with type badges, delta_qty, date; type/date filters; detail modal. i18n zh-CN+en |
| P3-35 | P2 | gRPC server bootstrap + TaskService | pending | — | Bootstrap gRPC server on admin entry point (separate port). Generate Go code from proto/task/v1/task.proto. Implement TaskService gRPC (ListTasks, GetTask, AssignTask, CompleteTask) delegating to existing service layer. Add gRPC reflection |
| P3-36 | P2 | PDA: Order lookup / detail page | pending | — | New PDA page: scan/type order number → fetch order with lines → display status badge, type, priority, line item table (SKU, qty, fulfilled qty). APIs exist from P1-14. i18n zh-CN+en |
| P3-37 | P2 | Admin: Audit log viewer page | pending | — | Table with filters (action, user, resource, date range). Detail modal showing old/new values JSON. Audit log API exists from P2-15 (GET /api/v1/audit-logs). i18n zh-CN+en |

<!-- GROOMED round 61 on 2026-07-23: discovery refill — 2→10 pending. 3 P1 (Location/Zone UI, PDA receiving, repo integration tests) + 7 P2. -->
