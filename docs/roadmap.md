...
> **Strict cap: 10 pending max.** Implement rounds do NOT add tasks. DISCOVER (pending < 3) refills to 10. GROOM (pending ≥ 8, every 5 rounds) prunes excess. Current total: 99 tasks.
...
| P3-51 | P1 | PDA: Replenishment workflow page | pending | — | New PDA page at /replenish: scan low-stock picking location → system finds reserve stock via FEFO/FIFO → operator confirms source/destination/qty → completes replenish task with inventory transfer. Backend task type "replenish" + inventory transfer logic already exists (P3-21). Link from Scan hub. i18n zh-CN+en. |
| P3-52 | P1 | PDA: Transfer workflow page | pending | — | New PDA page at /transfer: scan source location → scan destination location → enter SKU/batch/qty → confirm → complete transfer task. Backend task type "transfer" + inventory move logic already exists (P3-21). Handles same-warehouse stock moves (zone-to-zone, location-to-location). Link from Scan hub. i18n zh-CN+en. |
| P3-53 | P2 | Docker multi-stage builds for admin + PDA services | pending | — | Create backend/cmd/admin/Dockerfile and backend/cmd/pda/Dockerfile: multi-stage (golang:1.22-alpine build → alpine:3.20 runtime). Add admin + PDA as services in docker-compose.yml with depends_on (postgres, redis), health checks, and env vars. Fix Makefile docker-build target. Enables fully containerized dev/staging. |
| P3-54 | P2 | GitHub Actions CI pipeline for backend | pending | — | New workflow .github/workflows/ci.yml: go build ./..., go test ./..., go vet ./..., golangci-lint run on push/PR to master. Add Docker image build verification step. Add status badge to README.md. Trigger excludes docs-only changes. |
| P3-55 | P2 | Frontend test infrastructure: Vitest + React Testing Library | pending | — | Install vitest, @testing-library/react, @testing-library/jest-dom, jsdom in both frontend/admin and frontend/pda. Configure vitest.config.ts. Write smoke tests (renders without crashing) for 3-4 key pages per app. Add test scripts to package.json. Run in CI. |
| P3-56 | P2 | API documentation with Swagger UI | pending | — | Add swaggo/swag annotations to handler structs (summary, params, success/error responses). Generate OpenAPI 3.0 spec via swag init. Serve Swagger UI at GET /api/docs on admin server. Document all major endpoint groups: warehouses, SKUs, inventory, orders, ASNs, tasks, waves, shipments, cycle-counts, auth, users, roles. |
...
<!-- DISCOVER round 86 on 2026-07-24: refilled from 2→10 pending. Added PDA workflows (replenishment, transfer — backend task types already supported), Docker multi-stage builds, CI pipeline, frontend test infra, Swagger API docs, seed data enhancement (unblocks full demo), and PDA MSW mocks + ESLint config (fixes broken PDA demo + broken lint). All new tasks directly build on completed work: no speculative Phase 5+ items. -->
