...
| P3-43 | P2 | API handler unit test expansion | cancelled | — | **Why cancelled:** Already have 29 handler integration tests + 34 repo integration tests (~90% coverage). Low ROI for additional unit tests. |
| P3-44 | P2 | Docker Compose: PostgreSQL integration test setup | cancelled | — | **Why cancelled:** Speculative infrastructure. Integration tests work well with mock repos; real PG testing adds complexity without proportional benefit. |
| P3-45 | P2 | PDA: Network error handling & retry UX | cancelled | — | **Why cancelled:** Premature polish. Add error boundaries and retry UX when there's a measured usability problem, not before. |

<!-- GROOM round 80 on 2026-07-23: pruned from 8→5 pending. Cancelled P3-43 (low ROI, covered by integration tests), P3-44 (speculative infra), P3-45 (premature polish). Kept P3-38–P3-42 as the 5 highest-impact tasks. -->
