...
| P3-46 | P1 | Dedicated ASN service: extract from OrderService | pending | — | Create ASNService with CreateASN, GetASN, ListASNs, UpdateASNStatus, ReceiveASNLine. Extract ASN handler methods into dedicated ASNHandler. OrderService delegates ASN ops to ASNService. No API changes — pure internal refactoring with backward compatibility. |
...
| P3-49 | P2 | Frontend API module extraction: dedicated api/*.ts per resource | pending | — | Extract inline client.get/post calls from Admin pages into dedicated modules (api/warehouses.ts, api/zones.ts, api/locations.ts, api/skus.ts, api/orders.ts, api/tasks.ts, api/asns.ts, api/waves.ts). Same for PDA (api/tasks.ts, api/asns.ts, api/inventory.ts). Consistent with existing api/users.ts pattern. |
...
<!-- DISCOVER round 86 on 2026-07-23: refilled from 2→10 pending. New P3-43–P3-50. Focus: complete outbound flow (shipment), inventory accuracy (cycle count), refactoring (ASN service, API modules), operational readiness (health check, settings). -->
