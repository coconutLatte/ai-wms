...
| P3-21 | P2 | PDA: Camera barcode scanning integration | pending | — | Wire @zxing/library (already installed) into BarcodeScanner component for real camera scanning; fix Login double /api/v1 prefix bug; add barcode lookup API call with result navigation (location→putaway task, SKU→inventory info) |
| P3-22 | P1 | Order→Task generation on confirm | pending | — | When order status transitions to "confirmed", auto-generate tasks: receiving+putaway for inbound orders (per line→location), pick+ship for outbound orders. Wire OrderService.UpdateOrderStatus to call TaskService.CreateTask. Deduplicate: don't re-generate if tasks already exist for the order |
| P3-23 | P1 | Task completion inventory effects + transaction audit | pending | — | Putaway completion: create or increment inventory at target location, record receipt transaction. Pick completion: decrement inventory, record pick transaction. Use TxManager for atomic inventory update + transaction recording. Respect "no negative qty" rule |
| P3-24 | P1 | Inventory reservation for order allocation | pending | — | Add ReserveInventory/UnreserveInventory to InventoryService. Use FEFO (perishable) / FIFO (non-perishable) queries to find best available inventory. On order line status→allocated: reserve qty (increment reserved_qty, decrement available). On cancel: unreserve. Validate reserved_qty ≤ available_qty before reservation |
| P3-25 | P1 | Role management API (service + handler) | pending | — | Create RoleService (ListRoles, GetRole, CreateRole, UpdateRole, DeleteRole) and RoleHandler with REST endpoints under /api/v1/roles. Roles currently only seedable — this makes them manageable via admin UI. Needed by P3-20 for role CRUD in frontend |
| P3-26 | P2 | Admin: Dashboard real data | pending | — | Wire inventory dashboard API (GET /inventory/dashboard) for summary stat cards; add order summary (counts by status) and task summary (counts by status) API calls. Replace all "—" placeholders with live data. Dashboard is the landing page and currently shows no real data |
